...
export async function fetchAPI<E>(
  resource: string,
  accept: string,
  partialInit?: RequestInit,
  onError?: (response: Response) => Promise<E | undefined>,
): Promise<
  | {
      error: false;
      response: Response;
...
> {
  ...
  if (!response.ok) {
    ...
    if (result === undefined) {
      throw new Error(
        `failed to fetch api resource ${resource}: ${response.url} - ${response.status} - ${response.statusText}`,
      );
    }
...
export async function fetchXML<T, E = never>(
  resource: string,
  partialInit?: RequestInit,
  onError?: (response: Response) => Promise<E | undefined>,
): Promise<T | E> {
  const { error, response } = await fetchAPI<E>(
    resource,
    "application/xml",
    partialInit,
    onError,
...
export async function fetchJSON<T, E = never>(
  resource: string,
  partialInit?: RequestInit,
  onError?: (response: Response) => Promise<E | undefined>,
): Promise<T | E> {
