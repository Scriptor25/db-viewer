...
export function PageView<T, E>({pageAction, renderAction, extra, className}: Readonly<Props<T, E>>) {
    ...
    useEffect(() => {
        pageAction(index, extra).then(({count, elements}) => {
            renderAction(elements).then(content => setData({count, content}));
        });
    ...
    if (count > 1) {
        ...
        if (index <= HALF_SKIP_PAGE_COUNT)
            ...
            for (let i = index - HALF_SKIP_PAGE_COUNT; i <= index + HALF_SKIP_PAGE_COUNT; ++i)
                indices.push(i);
        ...
        navigation = (
            <div className={styles.navigation}>
                ...
                <span>{indices[0] + 1} - {indices[indices.length - 1] + 1} / {count}</span>
            </div>
        );
    }
