...
export function DataTableField<T>({
  ...
  data,
  active,
}: Readonly<DataTableFieldProps<T>>) {
    ...
    switch (field.type) {
      ...
      case "tristate": {
        ...
        switch (data) {
          ...
          default:
            throw new Error(
              `invalid tristate data ${data} for states ${field[0]} -> ${field[1]} -> ${field[2]}`,
            );
        }
  ...
  return (
    <td className={`${className ?? ""} ${active ? styles.active : ""}`}>
      {content}
    </td>
  );
}
